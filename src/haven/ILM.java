        ...
        synchronized (oc) {
            for (Gob gob : oc) {
                ...
                switch (gob.resname().hashCode()) {
                    case HF:
                        gob.setattr((lum = new Lumin(gob, new Coord(0, 0), 200, 96)));
                        break;
                    case FIRE:
                        gob.setattr((lum = new Lumin(gob, new Coord(0, 0), 200, 192)));
                        break;
                    case TORCHPOST:
                        gob.setattr((lum = new Lumin(gob, new Coord(0, -28), 300, 192)));
                        break;
                    case CANDLETHINGY:
                        gob.setattr((lum = new Lumin(gob, new Coord(0, -28), 400, 192)));
                        break;
                    case SPECIAL: {
                        ...
                        if (gob.getneg() != null && p.getneg() != null && // default client seems like it needs this
                            ...
                            gob.setattr((lum = new Lumin(gob, new Coord(0, -15), 200, 192)));
                        break;
                    }
                }
                if (lum == null)
